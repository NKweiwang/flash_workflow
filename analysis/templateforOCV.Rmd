...
```{}
T = 100
results = matrix(NA,ncol = 22, nrow = T)
for(i in 1:T){
...
## code for the tuning parameters from OCV only change one parameter

try this one later.....

```{r}
CVPMD_softImpute=function(Y,c_s,K,fold = 10, method = "PMD",N_sim = 10){
  N = dim(Y)[1]
  P = dim(Y)[2]
    ...
    for(iter in 1:N_sim){
      ...
      for(i in 1:fold){
        if(method == "PMD"){
          ...
          # out = PMD(missing[i,,], K = K)
        }else{
          out = softImpute(Y, rank.max = K,lambda = c_s[t_s]) 
        }
...
softImpute.wrapper = function(Y,ngrids = 10,K,fold = 10){
  ...
  N = dim(Y)[1]
  P = dim(Y)[2]
  c_s = seq(0,10,len=ngrids)
  cvout = CVPMD_softImpute(Y,c_s,K ,fold , method = "softImpute")
  out = softImpute(Y, rank.max = K,lambda = cvout$opt_s) 
  residual_softImpute = Y -  out$u %*%  diag(out$d) %*% t(out$v)
  return(list(d = out$d, u = out$u, v = out$v, residual = residual_softImpute))
}
```
...
plot_res = function(output,title = "data",legend_position = "none"){
  ...
  methods = rep(c("flash","NBSFA",pmd_c,softImpute_c), each = N)
  df = data.frame(RMSE = rmse, Method = methods )
  p<-ggplot(df, aes(x=Method, y=RMSE, color=Method)) +
  geom_boxplot()+
  # geom_violin()+
...
pt = plot_res(PT_res,"Tumor data")
pt
PT_res = readRDS("../data/missingvalue/box_res_grids_sf_pmd/TEXT_prsdt_box.rds")
pp = plot_res(PT_res,"Text data")
pp
PT_res = readRDS("../data/missingvalue/box_res_grids_sf_pmd/BreastCancer_box.rds")
pb = plot_res(PT_res,"Text data")
pb
```
