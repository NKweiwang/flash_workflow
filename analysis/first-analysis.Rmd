...
Here we take the simpliest example: 2 by 2 matrix $\begin{pmatrix} a & b \\ c & d  \end{pmatrix}$

#### 2.1.1 BCV in 2 by 2 case

In the original BCV frame work, we can use $||A - B D_k^+C||_F^2$ as objective function. 

As I understand here, in the simpliest case, $\begin{pmatrix} a & b \\ c & d  \end{pmatrix}$ is $2 \times 2$ matrix and we assume the this matrix is a rank one matrix. So we can use $a = bd^{-1}c$ to "impute" (or "predict") the value of $a$.

For example, we know the the matrix is $\begin{pmatrix} a & 3 \\ 2 & 3  \end{pmatrix}$ with rank one, so we know $a = 2$.

#### 2.1.1 OCV in 2 by 2 case
...
But what if we solve the together as we discussed previously on Orthogonal Cross Validation (OCV)?
...
Then the MSE for each row and column permutation would be:

\begin{eqnarray}
MSE &=& MSE_1 + MSE_2 + MSE_3\\
 &=& ||\hat{Y}_{(1)} - Y_{(1)}||_2 + ||\hat{Y}_{(2)} - Y_{(2)}||_2 + ||\hat{Y}_{(3)} - Y_{(3)}||_2
\end{eqnarray}

#### 2.3 2 by 2 matrix again

Let's go back to 2 by 2 case:

The question is can we get better idea by solving problem (2.1) and (2.2) together (by taking the average of MSE)?
...
&& v v^T = I \\
&& \lambda >0 \\
&where& MSE_1 = ||a - u_0\lambda v_0||_F + ||d - u_1 \lambda v_1||_F\\
&and& MSE_2 = ||b - u_0\lambda v_1||_F + ||c - u_1 \lambda v_0||_F
\end{eqnarray}
